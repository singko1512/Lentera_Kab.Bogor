...
@section('title', 'LENTERA Kab Bogor - Dashboard Admin')
...
<style>
    ...
    .btn-dice {
        ...
        background: linear-gradient(135deg, #115cb9 0%, #3b82f6 100%);
        color: #fff;
        border: none;
        border-radius: 12px;
        font-size: 0.85rem;
    ...
    .monitor-link:hover {
        ...
        background: rgba(17, 92, 185, 0.15);
    }
    ...
    .member-selector:hover {
        border-color: var(--primary);
        box-shadow: 0 0 0 3px rgba(17, 92, 185,0.08);
    }
    ...
    .timeline-day.drag-over {
        border-color: var(--primary);
        box-shadow: 0 4px 16px rgba(17, 92, 185,0.12);
        transform: translateY(-1px);
    }
    ...
    .btn-module-collapse[aria-expanded="true"] {
        background: #eef2ff;
        border-color: rgba(17, 92, 185, 0.35);
        color: var(--primary);
        box-shadow: 0 2px 10px rgba(17, 92, 185,0.08);
    }
...
<div class="admin-wrap">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="fw-bold mb-0" style="font-size:1
                
                etter-spacing:-0.3px; color:var(--dark);">
                    {{ $isSuperAdmin ? 'Dashboard Super Admin LENTERA Kab Bogor' : 'Dashboard Admin LENTERA Kab Bogor' }}
                </h1>
                <p class="mb-0 text-muted" style="font-size:0.85rem;">
                    {{ $isSuperAdmin ? 'Akses penuh Layanan Integrasi Izin Riset dan Magang Kabupaten Bogor' : 'Kelola peserta magang, absensi, laporan, dan sertifikat' }}
                </p>
        ...
        <div class="d-flex align-items-center gap-3">
            @if($isSuperAdmin)
                @php
                    $instansiOptions = \App\Models\Instansi::orderBy('nama')->get();
                    $currentInstansi = session('superadmin_instansi_id');
                @endphp
                <form action="{{ route('admin.switch_instansi') }}" method="POST" class="m-0 d-flex align-items-center gap-2 bg-white px-3 py-2 border rounded-3 shadow-sm">
                    @csrf
                    <label for="instansi_id" class="form-label mb-0 fw-semibold text-primary" style="font-size:0.85rem;"><i class="fa-solid fa-building me-1"></i> Instansi:</label>
                    <select name="instansi_id" id="instansi_id" class="form-select form-select-sm border-0 bg-transparent fw-bold text-dark" style="cursor: pointer; box-shadow: none;" onchange="this.form.submit()">
                        <option value="">-- Semua Instansi --</option>
                        @foreach($instansiOptions as $instansi)
                            <option value="{{ $instansi->id }}" {{ (int)$currentInstansi === $instansi->id ? 'selected' : '' }}>{{ $instansi->nama }}</option>
                        @endforeach
                    </select>
                </form>
            @endif
            ...
            <form action="{{ route('admin.dashboard') }}" method="GET" id="filterForm">
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-end gap-3">
                    <div class="d-flex flex-wrap gap-3">
                        <div>
                            <div class="filter-label">Tampilan Dinas</div>
                            <select name="dinas_id" id="filter_dinas_id" class="filter-select" onchange="document.getElementById('filterForm').submit()" style="min-width:240px;">
                                <option value="">Semua Dinas</option>
                                @foreach ($dinasOptions as $dinasOption)
                                    <option value="{{ $dinasOption->id }}" {{ (string) $activeDinasId === (string) $dinasOption->id ? 'selected' : '' }}>
                                        {{ $dinasOption->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="bidang_id" class="filter-select" onchange="document.getElementById('filterForm').submit()" style="min-width:240px;">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <a href="{{ route('admin.rekap.excel', ['month' => $month, 'year' => $year] + ($activeBidangId ? ['bidang_id' => $activeBidangId] : []) + ($activeDinasId ? ['dinas_id' => $activeDinasId] : [])) }}" class="btn-export-excel">
                            <i class="fa-solid fa-file-excel"></i> Unduh Excel
                        </a>
                        <a href="{{ route('admin.rekap.pdf', ['month' => $month, 'year' => $year] + ($activeBidangId ? ['bidang_id' => $activeBidangId] : []) + ($activeDinasId ? ['dinas_id' => $activeDinasId] : [])) }}" class="btn-export-pdf">
    ...
    <div class="mb-4">
        <div class="admin-tabs" id="adminTab">
            ...
            @if ($isSuperAdmin)
                ...
                <button type="button" class="tab-btn {{ $activeAdminTab === 'dinas' ? 'active' : '' }}" data-tab="dinas">
                    <i class="fa-solid fa-building-user"></i> Kelola Dinas
                </button>
            @endif
        </div>
    </div>
        ...
        <div class="admin-card overflow-hidden">
            ...
            <div class="p-4 pb-0">
                <form action="{{ route('admin.dashboard') }}" method="GET" class="d-flex flex-wrap gap-3 align-items-center">
                    ...
                    @endif
                    @if ($activeDinasId)
                        <input type="hidden" name="dinas_id" value="{{ $activeDinasId }}">
                    @endif
            ...
            <div class="px-4 pb-4">
                <form action="{{ route('admin.dashboard') }}" method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="pembimbing_magang_id" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                <option value="" disabled selected>Pilih Pembimbing</option>
                                @foreach (\App\Models\Simalam\PembimbingMagang::all() as $pembimbingOption)
                                    <option value="{{ $pembimbingOption->id }}" data-bidang="{{ $pembimbingOption->bidang_id }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($u->tanggal_mulai_magang || $u->tanggal_selesai_magang)
                                                @php
                                                    $tglMulai = $u->tanggal_mulai_magang ? \Carbon\Carbon::parse($u->tanggal_mulai_magang) : null;
                                                    $tglSelesai = $u->tanggal_selesai_magang ? \Carbon\Carbon::parse($u->tanggal_selesai_magang) : null;
                                                @endphp
                                                <div class="fw-semibold" style="font-size:0.82rem;">
                                                    {{ $tglMulai ? $tglMulai->translatedFormat('d F Y') : '-' }}
                                                </div>
                                                <div class="text-muted" style="font-size:0.78rem;">
                                                    s/d {{ $tglSelesai ? $tglSelesai->translatedFormat('d F Y') : '-' }}
                                                </div>
                                                @if ($tglSelesai && $tglSelesai->lte(now(config('app.timezone'))))
                                                    <a href="{{ Route::has('sertifikat.show') ? route('sertifikat.show', \Illuminate\Support\Str::slug($u->nama ?? $u->name)) : route('absensi.admin.sertifikat.view', $u->id) }}" target="_blank" class="attachment-link mt-1">
                                                        <i class="fa-solid fa-certificate"></i> Sertifikat
                                                    </a>
                                                @endif
                                            @else
    ...
    <div class="tab-panel {{ $activeAdminTab === 'sertifikat' ? '' : 'd-none' }}" id="panel-sertifikat">
        <div class="admin-card overflow-hidden">
            ...
            <div class="px-4 pb-4">
                <div class="p-3.5 border rounded-3 bg-white shadow-sm" style="border-color:#e2e8f0!important; background:#f8fafc;">
                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                        <div>
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <span class="badge bg-primary bg-opacity-10 text-primary px-2.5 py-1 rounded-2 fw-semibold" style="font-size:0.8rem;">
                                    <i class="fa-solid fa-file-code me-1"></i> Template Sertifikat Aktif
                                </span>
                            ...
                            <div class="text-muted" style="font-size:0.75rem;">
                                Upload file template (<code>.html</code> / <code>.docx</code>) untuk mengganti desain sertifikat default sistem.
                            </div>
                        </div>
                        <div>
                            <form action="{{ route('absensi.admin.sertifikat.template.upload') }}" method="POST" enctype="multipart/form-data" class="d-flex align-items-center gap-2">
                                @csrf
                                <input type="file" name="certificate_template" class="form-control form-control-sm" accept=".html,.htm,.txt,.docx,.doc" required style="max-width:260px; font-size:0.8rem;">
                                <button type="submit" class="btn btn-primary btn-sm fw-semibold d-inline-flex align-items-center gap-1.5" style="padding:0.4rem 0.9rem; font-size:0.8rem; border-radius:6px;">
                        ...
                        <tbody>
                            @foreach ($sertifikatUsers as $index => $u)
                                @php
                                    $tglSelesai = $u->tanggal_selesai_magang ? \Carbon\Carbon::parse($u->tanggal_selesai_magang) : null;
                                    $tglMulai = $u->tanggal_mulai_magang ? \Carbon\Carbon::parse($u->tanggal_mulai_magang) : null;
                                    $eligibleCertificate = $tglSelesai && $tglSelesai->lte(now(config('app.timezone')));
                                    $namaUser = $u->name ?? $u->nama ?? '—';
                                    $bidangNama = $u->bidang_magang ?? ($u->bidang->name ?? '-');
                                @endphp
                                <tr>
                                    <td class="text-center text-muted fw-semibold" style="font-size:0.85rem;">{{ $index + 1 }}</td>
                                    <td class="fw-bold text-dark" style="font-size:0.88rem;">{{ $namaUser }}</td>
                                    <td class="text-muted" style="font-size:0.82rem;">{{ $u->email ?? '-' }}</td>
                                    <td>
                                        <span class="badge bg-light text-dark border px-2.5 py-1.5 fw-semibold" style="font-size:0.78rem;">
                                            {{ $bidangNama }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="fw-bold text-dark" style="font-size:0.82rem;">Generate otomatis</div>
                                        <div class="text-muted" style="font-size:0.75rem;">
                                            {{ $certificateTemplate['name'] ?? 'Template Default Sistem' }}
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        @if ($eligibleCertificate)
                                            <div class="d-flex align-items-center justify-content-center gap-1.5">
                                                <a href="{{ route('absensi.admin.sertifikat.preview', $u) }}" target="_blank" class="btn btn-sm btn-outline-primary font-semibold d-inline-flex align-items-center gap-1" style="padding:0.35rem 0.65rem; font-size:0.78rem; border-radius:6px;" title="Preview sertifikat di browser">
                                        ...
                                        @else
                                            <button class="btn btn-sm btn-outline-secondary font-semibold d-inline-flex align-items-center gap-1" disabled style="padding:0.35rem 0.65rem; font-size:0.78rem; border-radius:6px;">
                ...
                <div class="mt-3 p-3 bg-white rounded-3 border d-flex flex-wrap align-items-center justify-content-between gap-3 shadow-sm" style="border-color:var(--border)!important;">
                    <div class="d-flex align-items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <div class="text-muted" style="font-size:0.75rem;">
                                {{ $adminBidangScope ? 'Pengaturan ini berlaku untuk bidang ' . $adminBidangScope->nama . ' di halaman awal sistem.' : 'Pilih tampilan default jadwal di halaman awal sistem untuk bidang yang belum punya pengaturan khusus.' }}
                            </div>
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <form action="{{ route('admin.jadwal.landing_view') }}" method="POST" id="landingViewForm" class="d-flex align-items-center gap-2 m-0">
                            @csrf
                            @if ($activeBidangId)
                                <input type="hidden" name="bidang_id" id="inputLandingBidangId" value="{{ $activeBidangId }}">
                            @endif
                                    ...
                                    @foreach ($users as $u)
                                        ...
                                        <tr id="normal-user-row-{{ $u->id }}" class="normal-user-schedule-row" data-user-id="{{ $u->id }}" data-user-name="{{ strtolower($u->nama) }}" data-user-bidang="{{ strtolower($bidangNama) }}">
                                            <td style="padding: 0.85rem 1.25rem;">
                                                <div class="d-flex align-items-center gap-3">
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold" style="width:36px; height:36px; font-size:0.85rem; background:#dbeafe; color:#115cb9; flex-shrink:0;">
                                            ...
                                            @endforeach
                                            <td class="text-center" style="vertical-align:middle;">
                                                <span class="jumat-fixed" style="font-size:0.78rem; padding:0.32rem 0.75rem; border-radius:6px; font-weight:700; display:inline-block;">WFH</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="p-3 border-top" style="border-color:var(--border)!important;">
                        ...
                        <div class="progress-comparison mb-3">
                            ...
                            <div class="comparison-bar-row">
                                ...
                                <div class="comparison-bar-bg">
                                    <div class="comparison-bar-fill" style="width: {{ $project->progress_percentage }}%; background: #115cb9;"></div>
                                </div>
                        ...
                        <div class="mb-4">
                            <button type="button" class="btn-module-collapse d-flex justify-content-between align-items-center w-100 p-2.5 px-3 rounded-3" 
                                ...
                                <span class="d-flex align-items-center gap-2">
                                    <span class="d-inline-flex align-items-center justify-content-center rounded-2" style="width: 30px; height: 30px; background: rgba(17, 92, 185, 0.12); color: var(--primary);">
                                    ...
                                    <form action="{{ route('admin.project.module.store') }}" method="POST" class="js-module-create-form" 
                                        ...
                                        <div class="row g-2">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <input type="number" name="bobot" value="10" min="0" max="100" step="any" class="form-control form-control-admin w-100 bg-white" required>
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label-admin">Mulai <span class="text-danger">*</span></label>
                                    ...
                                    <div class="gantt-row">
                                        ...
                                        <div class="gantt-timeline-container flex-grow-1">
                                             <div class="gantt-bar" style="left: 0; width: 100%; background: #e2e8f0; border: 1px solid #cbd5e1; box-shadow: none; display: flex; align-items: center; justify-content: flex-start; padding-left: 0; position: absolute; overflow: hidden; height: 100%;">
                                                 <!-- Progress Fill -->
                                                 <div style="position: absolute; top:0; left:0; bottom:0; width: {{ $module->progress }}%; background: linear-gradient(135deg, #115cb9 0%, #3b82f6 100%); transition: width 0.3s ease;"></div>
                        ...
                        <tbody id="tbodyPembimbing">
                            @foreach ($pembimbingMagangs as $index => $pembimbing)
                                <tr class="pembimbing-row" data-name="{{ strtolower($pembimbing->nama) }}" data-bidang="{{ strtolower($pembimbing->bidang->nama ?? '') }}">
                                    ...
                                    <td class="fw-semibold" style="color:var(--dark);">
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="avatar-circle-sm rounded-circle d-flex align-items-center justify-content-center fw-bold" style="width:28px; height:28px; font-size:0.75rem; background:rgba(17, 92, 185,0.1); color:var(--primary); border:1px solid rgba(17, 92, 185,0.2);">
                                    ...
                                    <td class="text-end pe-4">
                                        <button type="button" class="btn-action me-1" onclick="editPembimbing({{ $pembimbing->id }}, {{ json_encode($pembimbing->nama) }}, {{ json_encode($pembimbing->bidang_id) }}, {{ json_encode($pembimbing->dinas_id) }})" title="Edit">
                                            <i class="fa-solid fa-pen"></i>
                                        </button>
    ...
    {{-- TAB: Kelola Dinas --}}
    <div class="tab-panel {{ $activeAdminTab === 'dinas' ? '' : 'd-none' }}" id="panel-dinas">
        {{-- Card 1: Daftar Dinas Pemerintahan --}}
        <div class="admin-card overflow-hidden">
            <div class="p-4 pb-0 d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    <h6 class="fw-bold mb-1" style="color:var(--dark);">
                        <i class="fa-solid fa-building-user me-1" style="color:var(--primary);"></i> Daftar Dinas Pemerintahan
                    </h6>
                    <p class="text-muted mb-0" style="font-size:0.8rem;">
                        Kelola data instansi dinas pemerintahan yang terdaftar di dalam sistem.
                    </p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <span class="badge bg-light border text-secondary fw-semibold px-2.5 py-1.5" style="font-size:0.75rem;">
                        {{ $dinasOptions->count() }} Dinas
                    </span>
                    <button type="button" class="btn-add" data-bs-toggle="modal" data-bs-target="#addDinasModal">
                        <i class="fa-solid fa-plus"></i> Tambah Dinas
                    </button>
                </div>
            </div>

            @if ($dinasOptions->isEmpty())
                <div class="empty-state">
                    <h6>Belum ada dinas terdaftar</h6>
                    <p>Tambahkan dinas baru untuk memulai pengelolaan multi-instansi.</p>
                </div>
            @else
                <div class="table-responsive mt-3">
                    <table class="data-table mb-0">
                        ...
                        <tbody>
                            @foreach ($dinasOptions as $index => $d)
                                <tr>
                                    <td class="text-muted fw-semibold text-center">{{ $index + 1 }}</td>
                                    <td class="fw-semibold" style="color:var(--dark);">
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="badge bg-light text-primary border p-2 rounded-2" style="font-size:0.75rem;">
                                    ...
                                    <td class="text-end pe-4">
                                        <button type="button" class="btn-action me-1" onclick="editDinas({{ $d->id }}, {{ json_encode($d->nama) }})" title="Edit">
                                            <i class="fa-solid fa-pen"></i>
                                        </button>
                                        @if($d->nama !== 'Dinas Komunikasi dan Informatika')
                                        <form action="{{ route('admin.dinas.destroy', $d->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dinas ini?')">
                                            @csrf
        ...
        {{-- Card 2: Daftar Akun Dinas --}}
        <div class="admin-card overflow-hidden mt-4">
            <div class="p-4 pb-0 d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    <h6 class="fw-bold mb-1" style="color:var(--dark);">
                        <i class="fa-solid fa-user-shield me-1" style="color:var(--primary);"></i> Daftar Akun Dinas (Admin Instansi)
                    </h6>
                    <p class="text-muted mb-0" style="font-size:0.8rem;">
                        Kelola akun admin masing-masing dinas pemerintahan untuk mengelola peserta magang mereka.
                    </p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <span class="badge bg-light border text-secondary fw-semibold px-2.5 py-1.5" style="font-size:0.75rem;">
                        {{ $dinasUsers->count() }} Akun
                    </span>
                    <button type="button" class="btn-add" data-bs-toggle="modal" data-bs-target="#addDinasUserModal">
                        <i class="fa-solid fa-plus"></i> Tambah Akun Dinas
                    </button>
                </div>
            </div>

            @if ($dinasUsers->isEmpty())
                <div class="empty-state">
                    <h6>Belum ada akun dinas terdaftar</h6>
                    <p>Tambahkan akun dinas baru agar perwakilan dinas dapat masuk dan mengelola anak magang mereka.</p>
                </div>
            @else
                <div class="table-responsive mt-3">
                    <table class="data-table mb-0">
                        ...
                        <tbody>
                            @foreach ($dinasUsers as $index => $du)
                                <tr>
                                    <td class="text-muted fw-semibold text-center">{{ $index + 1 }}</td>
                                    <td class="fw-semibold" style="color:var(--dark);">
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="avatar-circle-sm rounded-circle d-flex align-items-center justify-content-center fw-bold" style="width:28px; height:28px; font-size:0.75rem; background:rgba(17, 92, 185,0.1); color:var(--primary); border:1px solid rgba(17, 92, 185,0.2);">
                                                {{ strtoupper(substr($du->nama, 0, 1)) }}
                                            </div>
                                            <span>{{ $du->nama }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div class="fw-semibold text-dark">{{ $du->username }}</div>
                                        <div class="text-muted small">{{ $du->email }}</div>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge-status {{ $du->status_akun === 'aktif' ? 'badge-hadir' : 'badge-sakit' }}">
                                            {{ $du->status_akun }}
                                        </span>
                                    </td>
                                    <td class="text-end pe-4">
                                        <button type="button" class="btn-action me-1" onclick="editDinasUser({{ $du->id }}, {{ json_encode($du->nama) }}, {{ json_encode($du->username) }}, {{ json_encode($du->email) }}, {{ json_encode($du->dinas_id) }}, {{ json_encode($du->status_akun) }})" title="Edit">
                                            <i class="fa-solid fa-pen"></i>
                                        </button>
                                        <form action="{{ route('admin.dinas_user.destroy', $du->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun dinas ini?')">
                                            @csrf
                                            <button type="submit" class="btn-action danger" title="Hapus">
                                                <i class="fa-solid fa-trash"></i>
                                            </button>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('admin.user.store') }}" method="POST">
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                    @if ($isSuperAdmin)
                        <select name="dinas_id" id="add_dinas_id" class="form-select form-control-admin w-100 js-dinas-select" data-bidang-select="add_bidang_id" data-pembimbing-select="add_pembimbing_magang_id" required>
                            <option value="" disabled selected>Pilih dinas...</option>
                            @foreach ($dinasOptions as $dOption)
                                <option value="{{ $dOption->id }}">{{ $dOption->nama }}</option>
                            @endforeach
                        </select>
                    @else
                        <input type="text" class="form-control form-control-admin w-100" value="{{ Auth::user()->dinas->nama ?? '-' }}" disabled>
                        <input type="hidden" name="dinas_id" id="add_dinas_id" class="js-dinas-select" data-bidang-select="add_bidang_id" data-pembimbing-select="add_pembimbing_magang_id" value="{{ Auth::user()->dinas_id }}">
                    @endif
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                        <label class="form-label-admin">Bidang Magang <span class="text-danger" id="add_bidang_req_star">*</span></label>
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="pembimbing_magang_id" id="add_pembimbing_magang_id" class="form-select form-control-admin w-100 js-pembimbing-select" required disabled>
                            <option value="" disabled selected>Pilih dinas terlebih dahulu...</option>
                            @foreach ($pembimbingMagangs as $pembimbing)
                                <option value="{{ $pembimbing->id }}" data-bidang-id="{{ $pembimbing->bidang_id }}" data-dinas-id="{{ $pembimbing->dinas_id }}">{{ $pembimbing->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="modal-content p-4">
            ...
            <form action="" method="POST" id="editForm">
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                    @if ($isSuperAdmin)
                        <select name="dinas_id" id="e_dinas_id" class="form-select form-control-admin w-100 js-dinas-select" data-bidang-select="e_bidang_id" data-pembimbing-select="e_pembimbing_magang_id" required>
                    ...
                    @else
                        <input type="text" class="form-control form-control-admin w-100" value="{{ Auth::user()->dinas->nama ?? '-' }}" disabled>
                        <input type="hidden" name="dinas_id" id="e_dinas_id" class="js-dinas-select" data-bidang-select="e_bidang_id" data-pembimbing-select="e_pembimbing_magang_id" value="{{ Auth::user()->dinas_id }}">
                    @endif
                </div>
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                        <label class="form-label-admin">Grup / Tim <span class="text-danger">*</span></label>
                        <select name="grup" id="e_grup" class="form-select form-control-admin w-100" required>
                            @foreach ($availableTeams as $tOption)
                                <option value="{{ $tOption }}">Tim {{ $tOption }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                        <label class="form-label-admin">Bidang Magang <span class="text-danger" id="e_bidang_req_star">*</span></label>
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="pembimbing_magang_id" id="e_pembimbing_magang_id" class="form-select form-control-admin w-100 js-pembimbing-select" required disabled>
                            <option value="" disabled>Pilih dinas terlebih dahulu...</option>
                            @foreach ($pembimbingMagangs as $pembimbing)
                                <option value="{{ $pembimbing->id }}" data-bidang-id="{{ $pembimbing->bidang_id }}" data-dinas-id="{{ $pembimbing->dinas_id }}">{{ $pembimbing->nama }}</option>
                            @endforeach
                        </select>
            ...
            <form action="" method="POST" id="editTimelineForm">
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="et_status" class="form-select form-control-admin w-100" required>
                        @foreach (\App\Models\Simalam\ProjectTimeline::statusOptions() as $statusValue => $statusLabel)
        ...
        <div class="modal-content p-4">
            ...
            <form action="" method="POST" id="editModuleForm">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label-admin">Bobot (%) <span class="text-danger">*</span></label>
                        <input type="number" name="bobot" id="em_bobot" min="0" max="100" step="any" class="form-control form-control-admin w-100" required>
                    </div>
                    <div class="col-md-6">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('admin.pembimbing.store') }}" method="POST">
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                    <select name="dinas_id" id="add_pembimbing_dinas_id" class="form-select form-control-admin w-100 js-dinas-select" data-bidang-select="add_pembimbing_bidang_id" data-pembimbing-select="" required>
                        <option value="" disabled selected>Pilih dinas...</option>
                        @foreach ($dinasOptions as $dOption)
                            <option value="{{ $dOption->id }}">{{ $dOption->nama }}</option>
                        @endforeach
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Bidang Magang <span class="text-danger" id="add_pembimbing_bidang_req_star">*</span></label>
                    <select name="bidang_id" id="add_pembimbing_bidang_id" class="form-select form-control-admin w-100" required>
                        <option value="" disabled selected>Pilih bidang...</option>
                        @foreach ($bidangs as $b)
                            <option value="{{ $b->id }}">{{ $b->nama }}</option>
                        @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="" method="POST" id="editPembimbingForm">
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                    <select name="dinas_id" id="epm_dinas_id" class="form-select form-control-admin w-100 js-dinas-select" data-bidang-select="epm_bidang_id" data-pembimbing-select="" required>
                        <option value="" disabled>Pilih dinas...</option>
                        @foreach ($dinasOptions as $dOption)
                            <option value="{{ $dOption->id }}">{{ $dOption->nama }}</option>
                        @endforeach
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Bidang Magang <span class="text-danger" id="epm_bidang_req_star">*</span></label>
...
{{-- Modal: Tambah Dinas --}}
<div class="modal fade modal-clean" id="addDinasModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            <h5 class="fw-bold mb-3"><i class="fa-solid fa-plus me-2" style="color:var(--primary);"></i>Tambah Dinas Pemerintahan</h5>
            <form action="{{ route('admin.dinas.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="form-label-admin">Nama Dinas <span class="text-danger">*</span></label>
                    <input type="text" name="nama" class="form-control form-control-admin w-100" placeholder="Contoh: Dinas Kesehatan" required>
                </div>
                <div class="d-flex gap-2">
                    <button type="submit" class="btn-add flex-grow-1 justify-content-center">Simpan</button>
                    <button type="button" class="btn-logout" data-bs-dismiss="modal">Batal</button>
...
{{-- Modal: Edit Dinas --}}
<div class="modal fade modal-clean" id="editDinasModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            <h5 class="fw-bold mb-3"><i class="fa-solid fa-pen me-2" style="color:var(--primary);"></i>Edit Dinas Pemerintahan</h5>
            <form action="" method="POST" id="editDinasForm">
                @csrf
                <div class="mb-4">
                    <label class="form-label-admin">Nama Dinas <span class="text-danger">*</span></label>
                    <input type="text" name="nama" id="edit_dinas_nama" class="form-control form-control-admin w-100" placeholder="Contoh: Dinas Kesehatan" required>
                </div>
                <div class="d-flex gap-2">
                    <button type="submit" class="btn-add flex-grow-1 justify-content-center">Simpan</button>
                    <button type="button" class="btn-logout" data-bs-dismiss="modal">Batal</button>
...
<div class="modal fade modal-clean" id="addDinasUserModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            <h5 class="fw-bold mb-3"><i class="fa-solid fa-user-plus me-2" style="color:var(--primary);"></i>Tambah Akun Dinas</h5>
            <form action="{{ route('admin.dinas_user.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="form-label-admin">Nama Lengkap <span class="text-danger">*</span></label>
                    <input type="text" name="nama" class="form-control form-control-admin w-100" placeholder="Contoh: Budi Santoso" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Username <span class="text-danger">*</span></label>
                    <input type="text" name="username" class="form-control form-control-admin w-100" placeholder="Contoh: admin.dinkes" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Email <span class="text-danger">*</span></label>
                    <input type="email" name="email" class="form-control form-control-admin w-100" placeholder="dinkes@email.com" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Password <span class="text-danger">*</span></label>
                    <input type="password" name="password" class="form-control form-control-admin w-100" placeholder="Minimal 6 karakter" required>
                ...
                <div class="mb-4">
                    <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                    <select name="dinas_id" class="form-select form-control-admin w-100" required>
                        <option value="" disabled selected>Pilih dinas...</option>
                        @foreach ($dinasOptions as $dOption)
                            <option value="{{ $dOption->id }}">{{ $dOption->nama }}</option>
                        @endforeach
...
<div class="modal fade modal-clean" id="editDinasUserModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            <h5 class="fw-bold mb-3"><i class="fa-solid fa-user-pen me-2" style="color:var(--primary);"></i>Edit Akun Dinas</h5>
            <form action="" method="POST" id="editDinasUserForm">
                @csrf
                <div class="mb-3">
                    <label class="form-label-admin">Nama Lengkap <span class="text-danger">*</span></label>
                    <input type="text" name="nama" id="edit_du_nama" class="form-control form-control-admin w-100" placeholder="Contoh: Budi Santoso" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Username <span class="text-danger">*</span></label>
                    <input type="text" name="username" id="edit_du_username" class="form-control form-control-admin w-100" placeholder="Contoh: admin.dinkes" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Email <span class="text-danger">*</span></label>
                    <input type="email" name="email" id="edit_du_email" class="form-control form-control-admin w-100" placeholder="dinkes@email.com" required>
                </div>
                <div class="mb-3">
                    <label class="form-label-admin">Password Baru</label>
                    <input type="password" name="password" id="edit_du_password" class="form-control form-control-admin w-100" placeholder="Kosongkan jika tidak diubah">
                </div>
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                        <label class="form-label-admin">Instansi Dinas <span class="text-danger">*</span></label>
                        <select name="dinas_id" id="edit_du_dinas_id" class="form-select form-control-admin w-100" required>
                            <option value="" disabled>Pilih dinas...</option>
                            @foreach ($dinasOptions as $dOption)
                                <option value="{{ $dOption->id }}">{{ $dOption->nama }}</option>
                            @endforeach
                    ...
                    <div class="col-sm-6">
                        <label class="form-label-admin">Status Akun <span class="text-danger">*</span></label>
                        <select name="status_akun" id="edit_du_status_akun" class="form-select form-control-admin w-100" required>
                            <option value="aktif">Aktif</option>
                            <option value="nonaktif">Nonaktif</option>
                        </select>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const panels = {
        ...
        dinas: document.getElementById('panel-dinas'),
        @endif
    };
...
function editUser(id, nama, email, pembimbingMagangId, bidangId, tanggalMulaiMagang, tanggalSelesaiMagang, statusAkun, grup, dinasId) {
    ...
    const dinasSelect = document.getElementById('e_dinas_id');
    if (dinasSelect) {
        dinasSelect.value = dinasId || '';
        updateDinasFiltering(dinasSelect);
    }
    ...
    const pembimbingSelect = document.getElementById('e_pembimbing_magang_id');
    if (pembimbingSelect) {
        pembimbingSelect.value = pembimbingMagangId || '';
    }
...
function submitPost(url) {
    ...
    token.type = 'hidden';
    token.name = '_token';
    token.value = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '{{ csrf_token() }}';
...
function confirmDel(e, url) {
    ...
    Swal.fire({
        ...
        cancelButtonText: 'Batal',
        customClass: { popup: 'rounded-4 border-0 shadow-lg', confirmButton: 'rounded-3', cancelButton: 'rounded-3' }
    }).then(r => { if (r.isConfirmed) submitPost(url); });
}
...
function editPembimbing(id, nama, bidangId, dinasId) {
    ...
    const dinasSelect = document.getElementById('epm_dinas_id');
    if (dinasSelect) {
        dinasSelect.value = dinasId || '';
        updateDinasFiltering(dinasSelect);
    }
...
function editDinas(id, nama) {
    const form = document.getElementById('editDinasForm');
    form.action = "{{ url('admin/dinas/update') }}/" + id;
    document.getElementById('edit_dinas_nama').value = nama;

    new bootstrap.Modal(document.getElementById('editDinasModal')).show();
}

function editDinasUser(id, nama, username, email, dinasId, statusAkun) {
    const form = document.getElementById('editDinasUserForm');
    form.action = "{{ url('admin/dinas-user/update') }}/" + id;
    document.getElementById('edit_du_nama').value = nama;
    document.getElementById('edit_du_username').value = username;
    document.getElementById('edit_du_email').value = email;
    document.getElementById('edit_du_dinas_id').value = dinasId || '';
    document.getElementById('edit_du_status_akun').value = statusAkun || 'aktif';
    document.getElementById('edit_du_password').value = '';

    new bootstrap.Modal(document.getElementById('editDinasUserModal')).show();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.updateLandingViewSetting = async function(mode) {
        ...
        try {
            ...
            const bidangId = document.getElementById('inputLandingBidangId')?.value || null;
            const payload = { jadwal_landing_view: mode };
            if (bidangId) {
                payload.bidang_id = bidangId;
            }
    ...
    window.diskominfoId = {{ \DB::table('md_dinas')->where('nama', 'Dinas Komunikasi dan Informatika')->value('id') ?? 0 }};

    window.updateDinasFiltering = function(dinasSelect) {
        const dinasId = parseInt(dinasSelect.value);
        const bidangSelectId = dinasSelect.getAttribute('data-bidang-select');
        const pembimbingSelectId = dinasSelect.getAttribute('data-pembimbing-select');

        const bidangSelect = bidangSelectId ? document.getElementById(bidangSelectId) : null;
        const pembimbingSelect = pembimbingSelectId ? document.getElementById(pembimbingSelectId) : null;

        const isDiskominfo = dinasId === window.diskominfoId;

        // Update Bidang Select requirements & visibility
        if (bidangSelect) {
            const bidangLabelReq = document.getElementById(bidangSelectId === 'add_bidang_id' ? 'add_bidang_req_star' : 
                                   (bidangSelectId === 'e_bidang_id' ? 'e_bidang_req_star' : 
                                   (bidangSelectId === 'add_pembimbing_bidang_id' ? 'add_pembimbing_bidang_req_star' : 'epm_bidang_req_star')));
            
            if (isDiskominfo) {
                bidangSelect.disabled = false;
                bidangSelect.required = true;
                if (bidangLabelReq) bidangLabelReq.classList.remove('d-none');
                bidangSelect.closest('.col-sm-6')?.classList.remove('d-none');
                bidangSelect.closest('.mb-4')?.classList.remove('d-none');
            } else {
                bidangSelect.value = "";
                bidangSelect.disabled = true;
                bidangSelect.required = false;
                if (bidangLabelReq) bidangLabelReq.classList.add('d-none');
                bidangSelect.closest('.col-sm-6')?.classList.add('d-none');
                bidangSelect.closest('.mb-4')?.classList.add('d-none');
            }
        }

        // Filter Pembimbing Options
        if (pembimbingSelect) {
            const pembimbingOptions = pembimbingSelect.querySelectorAll('option');
            let visibleCount = 0;

            pembimbingOptions.forEach(opt => {
                if (opt.value === "") return;
                const optDinasId = parseInt(opt.getAttribute('data-dinas-id'));
                const optBidangId = parseInt(opt.getAttribute('data-bidang-id'));

                let show = optDinasId === dinasId;
                if (show && isDiskominfo && bidangSelect && bidangSelect.value) {
                    show = optBidangId === parseInt(bidangSelect.value);
                }
            ...
            // Set default text for Pembimbing select
            const defaultOpt = pembimbingOptions[0];
            if (isNaN(dinasId)) {
                defaultOpt.textContent = "Pilih dinas terlebih dahulu...";
                pembimbingSelect.disabled = true;
                pembimbingSelect.value = "";
            } else if (visibleCount === 0) {
                defaultOpt.textContent = "Belum ada pembimbing di dinas ini...";
                pembimbingSelect.disabled = true;
                pembimbingSelect.value = "";
            } else {
                defaultOpt.textContent = isDiskominfo && bidangSelect && !bidangSelect.value ? "Pilih bidang terlebih dahulu..." : "Pilih pembimbing...";
                pembimbingSelect.disabled = isDiskominfo && bidangSelect && !bidangSelect.value;
                
                // Keep current value if it is still valid/visible
                const currentSelectedOpt = pembimbingSelect.querySelector(`option[value="${pembimbingSelect.value}"]`);
                if (currentSelectedOpt && (currentSelectedOpt.classList.contains('d-none') || currentSelectedOpt.disabled)) {
                    pembimbingSelect.value = "";
                }
            }
        }
    ...
    // Attach listeners
    document.querySelectorAll('.js-dinas-select').forEach(select => {
        select.addEventListener('change', function() {
            window.updateDinasFiltering(this);
        });
        // Run initial filtering if loaded with value
        if (select.value) {
            window.updateDinasFiltering(select);
        }
    });

    document.querySelectorAll('.js-bidang-select').forEach(select => {
        select.addEventListener('change', function() {
            const targetPembimbingId = this.getAttribute('data-pembimbing-target');
            const pembimbingSelect = document.getElementById(targetPembimbingId);
            const dinasSelect = this.closest('form').querySelector('.js-dinas-select');
            if (dinasSelect) {
                window.updateDinasFiltering(dinasSelect);
            }
        });
    });
});

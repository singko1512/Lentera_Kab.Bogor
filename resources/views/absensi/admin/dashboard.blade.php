...
<style>
    ...
    #panel-sertifikat .data-table th:nth-child(6),
    #panel-sertifikat .data-table td:nth-child(6) {
        min-width: 130px;
    }
...
<div class="admin-wrap">
    ...
    <div class="admin-card p-4 mb-4">
        <form action="{{ route($dashboardRouteName) }}" method="GET" id="filterForm">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-end gap-3">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('absensi.admin.rekap.excel', ['month' => $month, 'year' => $year] + ($activeBidangId ? ['bidang_id' => $activeBidangId] : [])) }}" class="btn-export-excel">
                        <i class="fa-solid fa-file-excel"></i> Unduh Excel
                    </a>
                    <a href="{{ route('absensi.admin.rekap.pdf', ['month' => $month, 'year' => $year] + ($activeBidangId ? ['bidang_id' => $activeBidangId] : [])) }}" class="btn-export-pdf">
    ...
    <div class="tab-panel {{ $activeAdminTab === 'rekap' ? '' : 'd-none' }}" id="panel-rekap">
        <div class="admin-card overflow-hidden">
            ...
            <div class="p-4 pb-0">
                <form action="{{ route($dashboardRouteName) }}" method="GET" class="d-flex flex-wrap gap-3 align-items-center">
                            ...
                            <tbody>
                                @foreach ($absensiRecords as $rec)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ route('absensi.admin.absensi.destroy', $rec) }}" method="POST" class="d-inline">
                                                @csrf
                                                <input type="hidden" name="month" value="{{ $month }}">
                                                <input type="hidden" name="year" value="{{ $year }}">
                                                <input type="hidden" name="search" value="{{ $search }}">
    ...
    <div class="tab-panel {{ $activeAdminTab === 'sertifikat' ? '' : 'd-none' }}" id="panel-sertifikat">
        <div class="admin-card overflow-hidden">
            <div class="p-4 d-flex flex-wrap justify-content-between align-items-center gap-2 border-bottom mb-3" style="border-color:var(--border)!important;">
                <div>
    ...
    <div class="tab-panel {{ $activeAdminTab === 'jadwal' ? '' : 'd-none' }}" id="panel-jadwal">
        ...
        <div class="admin-card overflow-hidden">
            ...
            <div class="p-4 border-bottom bg-white" style="border-color:var(--border)!important;">
                ...
                <div class="mt-3 p-3 bg-white rounded-3 border d-flex flex-wrap align-items-center justify-content-between gap-3 shadow-sm" style="border-color:var(--border)!important;">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <form action="{{ route('absensi.admin.jadwal.landing_view') }}" method="POST" id="landingViewForm" class="d-flex align-items-center gap-2 m-0">
                            @csrf
                            @if ($activeBidangId)
            ...
            <div id="jadwal-view-normal" class="{{ request('view', 'normal') !== 'team' ? '' : 'd-none' }}">
                <div class="p-4 border-bottom bg-light bg-opacity-50" style="border-color:var(--border)!important;">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <form action="{{ route('absensi.admin.jadwal.random') }}" method="POST" onsubmit="return confirmRandomSchedule(event)" class="d-inline">
                                @csrf
                ...
                @else
                    <form action="{{ route('absensi.admin.jadwal.update') }}" method="POST" id="scheduleForm">
                        @csrf
                        <div class="table-responsive">
            ...
            <div id="jadwal-view-team" class="{{ request('view') === 'team' ? '' : 'd-none' }}">
                ...
                @else
                    ...
                    <form id="randomTeamMembersForm" action="{{ route('absensi.admin.jadwal.team.random_members') }}" method="POST" onsubmit="return confirmRandomTeamMembers(event)" class="d-none">
                        @csrf
                    </form>
                    <form id="randomTeamScheduleForm" action="{{ route('absensi.admin.jadwal.team.random') }}" method="POST" onsubmit="return confirmRandomTeamSchedule(event)" class="d-none">
                        @csrf
                    </form>

                    {{-- Card 2: Form Kelola & Pembagian Tim Peserta (Master dari Kelola Magang) --}}
                    <div class="p-4">
                        <form action="{{ route('absensi.admin.jadwal.team.members') }}" method="POST" id="teamMembersForm">
                            @csrf
    ...
    <div class="tab-panel {{ $activeAdminTab === 'timeline' ? '' : 'd-none' }}" id="panel-timeline">
        <div class="admin-card p-4 mb-4">
            <form action="{{ route('absensi.admin.project.store') }}" method="POST" id="createProjectForm">
                @csrf
                    ...
                    @foreach ($pendingTasks as $task)
                        <div class="monitor-item" style="border-left: 5px solid #f59e0b !important;">
                            ...
                            <div class="monitor-actions mt-3">
                                <form action="{{ route('absensi.admin.project.task.approve', $task) }}" method="POST" class="d-inline">
                                    @csrf
                                ...
                                <form action="{{ route('absensi.admin.project.task.revision', $task) }}" method="POST" class="d-flex gap-2 flex-grow-1">
                                    @csrf
                ...
                <div class="project-row" id="project-row-{{ $project->id }}">
                    <div class="project-row-header">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            ...
                            <form action="{{ route('absensi.admin.project.destroy', $project) }}" method="POST" class="d-inline">
                                @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <div class="collapse mt-2" id="collapseAddModule-{{ $project->id }}">
                                <div class="p-3 border rounded-3 shadow-sm" style="border-color: var(--border) !important; background: #fbfcfe !important;">
                                    <form action="{{ route('absensi.admin.project.module.store') }}" method="POST" class="js-module-create-form" 
                                ...
                                @foreach ($projectModules as $module)
                                    <div class="monitor-item bg-white p-3 rounded-3 shadow-sm border" style="border-color: var(--border) !important;">
                                                        ...
                                                        <div class="p-2 rounded-2 border bg-light" style="font-size:0.73rem;">
                                                            <div class="d-flex justify-content-between align-items-start gap-1">
                                                                ...
                                                                <div class="d-flex align-items-center gap-1">
                                                                    ...
                                                                    @endif
                                                                    <form action="{{ route('absensi.admin.project.task.destroy', $task) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tugas ini?')">
                                                                        @csrf
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-3 pt-2 border-top" style="border-color:var(--border)!important;">
                                            ...
                                            <form action="{{ route('absensi.admin.project.module.destroy', $module) }}" method="POST">
                                                @csrf
                                        ...
                                        <div class="col-md-6 col-lg-4">
                                            <div class="p-2.5 border rounded bg-light position-relative">
                                                <div class="d-flex justify-content-between align-items-start mb-1">
                                                    ...
                                                    <form action="{{ route('absensi.admin.project.task.destroy', $task) }}" method="POST" class="d-inline">
                                                        @csrf
                                                ...
                                                @endif
                                                <form action="{{ route('absensi.admin.project.task.assign_pic', $task) }}" method="POST" class="mt-2 d-flex gap-1 align-items-center">
                                                    @csrf
                                        ...
                                        @forelse ($projectTasks->whereIn('status', ['sedang_dikerjakan', 'revision']) as $task)
                                            <div class="kanban-card position-relative" style="border-left: 4px solid {{ $task->status === 'revision' ? '#ef4444' : '#3b82f6' }} !important;">
                                                ...
                                                <div class="text-muted mt-1 d-flex align-items-center gap-1" style="font-size:0.74rem;">
                                                    Penanggung jawab: <strong class="text-dark">{{ $task->user->nama ?? '-' }}</strong>
                                                    <form action="{{ route('absensi.admin.project.task.unassign_pic', $task) }}" method="POST" class="d-inline">
                                                        @csrf
                            ...
                            @foreach ($bidangs as $b)
                                <tr class="bidang-row" data-name="{{ strtolower($b->nama) }}">
                                    ...
                                    <td class="text-end pe-4">
                                        ...
                                        <a href="#" class="btn-action danger" onclick="confirmDelBidang(event, '{{ route('absensi.admin.bidang.destroy', $b->id) }}')" title="Hapus">
                                            <i class="fa-solid fa-trash"></i>
                                        </a>
                                    </td>
                                </tr>
                            ...
                            @foreach ($pembimbingMagangs as $index => $pembimbing)
                                <tr class="pembimbing-row" data-name="{{ strtolower($pembimbing->nama) }}" data-bidang="{{ strtolower($pembimbing->bidang->nama ?? '') }}">
                                    ...
                                    <td class="text-end pe-4">
                                        ...
                                        <a href="#" class="btn-action danger" onclick="confirmDelPembimbing(event, '{{ route('absensi.admin.pembimbing.destroy', $pembimbing->id) }}')" title="Hapus">
                                            <i class="fa-solid fa-trash"></i>
                                        </a>
                                    </td>
                                </tr>
...
<div class="modal fade modal-clean" id="modalTambahTim" tabindex="-1" aria-labelledby="modalTambahTimLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.jadwal.team.store') }}" method="POST">
                @csrf
                <div class="mb-3">
...
<div class="modal fade modal-clean" id="addModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.user.store') }}" method="POST">
                @csrf
                <div class="mb-3">
...
<div class="modal fade modal-clean" id="noteModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.project.note.store') }}" method="POST">
                @csrf
...
<div class="modal fade modal-clean" id="addBidangModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.bidang.store') }}" method="POST">
                @csrf
                <div class="mb-4">
...
<div class="modal fade modal-clean" id="addPembimbingModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.pembimbing.store') }}" method="POST">
                @csrf
                <div class="mb-4">
...
<div class="modal fade modal-clean" id="addTaskModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content p-4">
            ...
            <form action="{{ route('absensi.admin.project.task.store') }}" method="POST" id="addTaskForm">
                @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (activeTab && panels[activeTab]) {
        ...
        const defaultActive = document.querySelector('#adminTab .tab-btn.active') || tabs[0];
        if (defaultActive) updateTabIndicator(defaultActive);
    }

    window.addEventListener('resize', () => {
        const currentActive = document.querySelector('#adminTab .tab-btn.active');
...
async function saveDayAssignment(day, employee) {
    try {
        const response = await fetch("{{ route('absensi.admin.project.assignment.store') }}", {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    teamSelects.forEach(select => {
        select.addEventListener('change', async function() {
            ...
            try {
                const response = await fetch("{{ route('absensi.admin.jadwal.team.members') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    window.updateLandingViewSetting = async function(mode) {
        ...
        try {
            ...
            const res = await fetch("{{ route('absensi.admin.jadwal.landing_view') }}", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': token,

    ...
    public function up(): void
    {
        ...
        Schema::table('md_pembimbing_magang', function (Blueprint $table) {
            if (! Schema::hasColumn('md_pembimbing_magang', 'bidang_id')) {
                $table->foreignId('bidang_id')->nullable()->after('id')->constrained('bidang')->nullOnDelete();
            }
        });

        if (! Schema::hasTable('users') || ! Schema::hasTable('bidang')) {
            return;
        }
        ...
        DB::table('md_pembimbing_magang')
            ...
            ->each(function ($pembimbing): void {
                ...
                $bidangId = DB::table('bidang')->where('nama', $bidangName)->value('id');

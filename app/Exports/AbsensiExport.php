...
class AbsensiExport implements FromArray, ShouldAutoSize, WithEvents
{
    ...
    public function array(): array
    {
        ...
        foreach ($users as $user) {
            $hadir = $user->absensi ? $user->absensi->where('status', 'hadir')->count() : 0;
            $wfh = $user->absensi ? $user->absensi->where('status', 'wfh')->count() : 0;
            $sakit = $user->absensi ? $user->absensi->where('status', 'sakit')->count() : 0;
            $izin = $user->absensi ? $user->absensi->where('status', 'izin')->count() : 0;
            ...
            $rows[] = [
                ...
                $user->nama ?: $user->name,
                $user->nip_atau_id ?? '-',
                $user->pembimbing_magang ?? ($user->pembimbingMagang?->nama ?? '-'),
                $user->bidang_magang ?? ($user->bidang?->nama ?? '-'),
                $hadir,
                $wfh,
                $sakit,
                $izin,

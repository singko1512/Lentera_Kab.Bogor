...
use App\Models\Simalam\ProjectDayAssignment;
use App\Models\Simalam\ProjectModule;
use App\Models\Simalam\ProjectNote;
use App\Models\Simalam\ProjectNoteReply;
use App\Models\Simalam\ProjectTask;
use App\Models\Simalam\ProjectTaskParticipant;
use App\Models\Simalam\ProjectTimeline;
use App\Models\User;
use App\Models\Simalam\WorkSubmission;
use Carbon\Carbon;
...
class ProjectTimelineController extends Controller
{
    public function storeProject(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])
    ...
    public function updateProject(Request $request, Project $project)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Timeline proyek berhasil diperbarui.');
    }
    ...
    public function destroyProject(Project $project)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Timeline proyek berhasil dihapus.');
    }
    ...
    public function storeTimeline(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Timeline proyek berhasil dibuat.');
    }
    ...
    public function storeModule(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Modul pekerjaan berhasil dibuat.');
    }
    ...
    public function updateModule(Request $request, ProjectModule $module)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Modul pekerjaan berhasil diperbarui.');
    }
    ...
    public function destroyModule(ProjectModule $module)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Modul pekerjaan berhasil dihapus.');
    }
    ...
    public function storeTask(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Tugas pekerjaan berhasil dibuat.');
    }
    ...
    public function assignTaskPIC(Request $request, ProjectTask $task)
    {
        ...
        if (! $task->project->members()->where('users.id', $data['user_id'])->exists()) {
    ...
    public function lockTask(ProjectTask $task)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Tugas tidak bisa diambil peserta untuk sementara.');
    }
    ...
    public function reopenTask(ProjectTask $task)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Tugas kembali bisa diambil peserta.');
    }
    ...
    public function approveSubmission(WorkSubmission $submission)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Laporan pekerjaan berhasil disetujui.');
    }
    ...
    public function revisionSubmission(Request $request, WorkSubmission $submission)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Laporan pekerjaan dikembalikan untuk revisi.');
    }
    ...
    public function storeNote(Request $request)
    {
        ...
        if ($tanggal < $project->tanggal_mulai->toDateString() || $tanggal > $project->tanggal_selesai->toDateString()) {
            return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('error_swal', 'Tanggal catatan harus berada di rentang proyek.');
        }

        $userId = isset($data['user_id']) ? (int) $data['user_id'] : null;

        if ($userId && ! $project->members->contains('id', $userId)) {
            return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('error_swal', 'Peserta magang pada catatan harus termasuk anggota proyek.');
        }
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Note timeline berhasil ditambahkan.');
    }
    ...
    public function completeNote(Request $request, ProjectNote $note)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Note berhasil ditandai selesai.');
    }
    ...
    private function ensureCanManageProject(Project $project): void
    {
        ...
        if (! $admin || ! in_array($admin->role, ['admin', 'superadmin', 'dinas', 'bidang'], true)) {
            abort(403);
        }

        if (in_array($admin->role, ['admin', 'superadmin', 'dinas'], true)) {
            return;
        }
    ...
    private function ensureCanUseUsers(array $userIds): void
    {
        ...
        if (! $admin || in_array($admin->role, ['admin', 'superadmin', 'dinas'], true)) {
            return;
        }
        ...
        $validCount = User::whereIn('role', ['peserta', 'user'])
    ...
    public function selfAssignTask(ProjectTask $task)
    {
        ...
        $userId = Auth::id();
        if ($project->members()->exists() && ! $project->members()->where('users.id', $userId)->exists()) {
    ...
    public function approveTask(ProjectTask $task)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Tugas berhasil disetujui.');
    }
    ...
    public function revisionTask(Request $request, ProjectTask $task)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'timeline'])->with('success_swal', 'Permintaan revisi berhasil dikirim.');
    }
}

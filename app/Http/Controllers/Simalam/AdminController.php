...
use App\Models\Simalam\JadwalMingguan;
use App\Models\Simalam\MasterData;
use App\Models\Simalam\PembimbingMagang;
...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        if (! in_array($activeAdminTab, $allowedTabs, true)) {
            return redirect()
                ->route('absensi.admin.dashboard', ['tab' => 'pegawai'])
        ...
        if ($dinasId) {
            $magangUsersQuery->where(function($q) use ($dinasId) {
        ...
        $users = $magangUsers->pluck('user')->filter()->unique('id')->values();
        if ($users->isEmpty()) {
        ...
        $magangGroups = $magangUsers->groupBy(function ($item) {
            return (isset($item->user) && $item->user->pembimbing_magang) ? $item->user->pembimbing_magang : 'Belum ada pembimbing';
        });
        ...
        // Auto-sync absensis table records to md_absensi table for the selected period
        $unSyncedAbsensis = DB::table('absensis')
            ->join('magang_applications', 'absensis.magang_application_id', '=', 'magang_applications.id')
            ->whereBetween('absensis.tanggal', [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
            ->select('absensis.*', 'magang_applications.user_id')
            ->get();

        foreach ($unSyncedAbsensis as $unSynced) {
            if ($unSynced->user_id) {
                $stId = MasterData::idFor(MasterData::ABSENSI_STATUS, $unSynced->status ?: 'hadir');
                Absensi::updateOrCreate(
                    [
                        'user_id' => $unSynced->user_id,
                        'tanggal' => $unSynced->tanggal,
                    ],
                    [
                        'jam_masuk' => $unSynced->waktu_masuk,
                        'jam_pulang' => $unSynced->waktu_pulang,
                        'status' => $unSynced->status ?: 'hadir',
                        'status_id' => $stId,
                        'foto_masuk' => $unSynced->foto_masuk,
                        'foto_pulang' => $unSynced->foto_pulang,
                        'laporan' => $unSynced->catatan,
                    ]
                );
            }
        }
        ...
        if ($activeBidangId) {
            ...
            $sertifikatUsersQuery->where('dinas_id', $dinasId);
        }
    ...
    public static function getAvailableTeams(): array
    {
        ...
        $userTeams = User::whereIn('role', ['peserta', 'user'])
            ->whereNotNull('grup')
            ->where('grup', '!=', '')
            ->distinct()
            ->pluck('grup')
    ...
    private function currentAdminBidangScope(): ?Bidang
    {
        ...
        if (! $admin || in_array($admin->role, ['admin', 'superadmin'], true) || ! $admin->bidang_id) {
            return null;
        }
    ...
    {
        $query = User::whereIn('role', ['peserta', 'user']);
    ...
    public function storeUser(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'pegawai'])->with('success_swal', 'Peserta magang baru berhasil ditambahkan!');
    }
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'pegawai'])->with('success_swal', 'Data peserta magang berhasil diperbarui!');
    }
    ...
    public function destroyUser($id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'pegawai'])->with('success_swal', 'Peserta magang berhasil dihapus!');
    }
    ...
    public function uploadSertifikat(Request $request, User $user)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'sertifikat'])
            ->with('success_swal', 'Sertifikat '.$user->nama.' berhasil diunggah.');
    }

    public function uploadSertifikatTemplate(Request $request)
    {
        if (! in_array(Auth::user()?->role, ['admin', 'superadmin', 'dinas', 'bidang'], true)) {
            abort(403);
        }

        $request->validate([
            'certificate_template' => 'required|file|mimes:html,htm,txt,docx,doc|max:5120',
        ], [
            'certificate_template.required' => 'File template sertifikat wajib dipilih.',
            'certificate_template.mimes' => 'Template sertifikat harus berupa file HTML, TXT, atau DOCX.',
            'certificate_template.max' => 'Ukuran template sertifikat maksimal 5 MB.',
        ]);
    ...
    public function previewSertifikat(User $user)
    {
        $this->ensureCanManageUser($user);
        ...
        if ($user->sertifikat_file_path && Storage::disk('local')->exists($user->sertifikat_file_path)) {
            ...
            $mime = $user->sertifikat_file_mime ?: 'application/pdf';
        ...
        return $pdf->stream(CertificatePayload::fileName($user));
    }
    ...
    public function updateSchedules(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'normal'])->with('success_swal', 'Jadwal mingguan berhasil disimpan!');
    }
    ...
    public function randomizeSchedules()
    {
        ...
        if ($users->isEmpty()) {
            return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'normal'])->with('error_swal', 'Belum ada peserta magang untuk dijadwalkan.');
        }
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'normal'])->with('success_swal', 'Jadwal mingguan normal berhasil diacak! Pola selang-seling pasangan (Senin-Rabu dan Selasa-Kamis) diterapkan, Jumat tetap WFH.');
    }
    ...
    public function storeTeam(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'team'])
    ...
    public function updateTeamSchedules(Request $request)
    {
        ...
        if ($users->isEmpty()) {
            return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'team'])->with('error_swal', 'Belum ada peserta magang untuk dijadwalkan.');
        }
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'normal'])->with('success_swal', 'Jadwal berbasis tim berhasil diterapkan secara serentak!');
    }
    ...
    public function randomizeTeamSchedules()
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'normal'])->with('success_swal', 'Jadwal tim berhasil diacak serentak! ('.$detail.', Jumat semua WFH).');
    }
    ...
    public function randomizeTeamMembers()
    {
        ...
        if ($users->isEmpty()) {
            return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'team'])->with('error_swal', 'Belum ada peserta magang untuk dibagi ke dalam tim.');
        }
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'team'])->with('success_swal', 'Anggota tim berhasil diacak secara merata ke seluruh tim yang tersedia!');
    }
    ...
    public function updateTeamMembers(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'jadwal', 'view' => 'team'])->with('success_swal', 'Pembagian tim peserta magang berhasil disimpan!');
    }
    ...
    public function updateLandingScheduleView(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', array_filter([
            'tab' => 'jadwal',
            'view' => 'normal',
            'bidang_id' => $bidangId,
    ...
    public function exportPdf(Request $request)
    {
        ...
        $employeesQuery = User::with(['absensi' => function ($query) use ($startDate, $endDate) {
            $query->whereBetween('tanggal', [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
                ->with('statusMaster')
                ->orderBy('tanggal', 'asc');
        }])->whereIn('role', ['peserta', 'user'])->orderBy('name', 'asc');
        ...
        $pdf = Pdf::loadView('absensi.admin.rekap_pdf', compact('rekapData', 'month', 'year', 'namaBulan', 'totalWorkdays'));
    ...
    public function updateBidang(Request $request, $id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'bidang'])->with('success_swal', 'Nama bidang berhasil diperbarui!');
    }
    ...
    public function destroyBidang($id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'bidang'])->with('success_swal', 'Bidang berhasil dihapus!');
    }
    ...
    public function storePembimbing(Request $request)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'bidang'])->with('success_swal', 'Pembimbing magang baru berhasil ditambahkan!');
    }
    ...
    public function updatePembimbing(Request $request, $id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'bidang'])->with('success_swal', 'Nama pembimbing berhasil diperbarui!');
    }
    ...
    public function destroyPembimbing($id)
    {
        ...
        return redirect()->route('absensi.admin.dashboard', ['tab' => 'bidang'])->with('success_swal', 'Pembimbing magang berhasil dihapus!');
    }
